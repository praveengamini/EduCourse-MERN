  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black to-gray-900 text-white">
      ...
      <div className="container mx-auto px-4 py-20">
        ...
        {/* Header Section */}
        <div className="bg-black/80 backdrop-blur-sm rounded-2xl shadow-xl p-8 mb-8 border border-white/20">
          <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
            <div>
              <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-700 to-indigo-500 bg-clip-text text-transparent mb-2">
                All Courses
              </h1>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="relative">
                ...
                <input
                  ...
                  value={filters.title}
                  onChange={handleFilterChange}
                  className="pl-10 pr-4 py-3 border border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-700 focus:border-transparent bg-gray-800 text-white shadow-sm transition-all duration-200"
                />
              </div>

              <div className="flex items-center gap-2 bg-gray-800/80 backdrop-blur-sm rounded-xl p-1">
                <FunnelIcon className="h-4 w-4 text-gray-400 ml-2" />
                <select
                  name="cost"
                  value={filters.cost}
                  onChange={handleFilterChange}
                  className="bg-gray-800 border-0 px-3 py-2 focus:outline-none text-sm font-medium text-white"
                >
                  <option value="">All Prices</option>
                  <option value=">1000">{'> ₹1000'}</option>
                  <option value="<2000">{'< ₹2000'}</option>
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-400">
              Showing <span className="font-semibold text-purple-500">{courses.length}</span> courses
            </p>
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
              Live updates
              ...
              >
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-sm rounded-full p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <ArrowLeftIcon className="h-4 w-4 text-purple-500 rotate-180" />
                  </div>
                </div>

                <div className="p-6">
                  <h2 className="text-lg font-bold text-white mb-3 group-hover:text-purple-500 transition-colors duration-200 line-clamp-2">
                    {course.title}
                  </h2>

                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center text-sm text-gray-300 bg-gray-800/80 rounded-full px-3 py-1">
                        <UsersIcon className="h-4 w-4 mr-2 text-purple-500" />
                        <span className="font-medium">{course.studentCount}</span>
                        <span className="ml-1">students</span>
                      </div>

                      <div className="flex items-center text-sm text-green-300 bg-green-900/20 rounded-full px-3 py-1">
                        <CurrencyRupeeIcon className="h-4 w-4 mr-1 text-green-400" />
                        <span className="font-bold">₹{course.cost}</span>
                      </div>
                    </div>

                    <div className="flex items-center text-xs text-gray-400 bg-gray-800/80 rounded-full px-3 py-2">
                      <CalendarDaysIcon className="h-4 w-4 mr-2" />
                      <span>
                ...
                {/* Hover Glow Effect */}
                <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-purple-700/10 to-purple-900/10"></div>
                </div>
              </div>
            ))}
          </div>
